function [embed_data,Seq_r,bits_frame] = encode_i_frame_lin(Seq,Quant)
...
global QP h w c Threshold zero_point
...
[h,w,u] = size(Seq);
QP = Quant;
% Load category of macroblocks in all videos
load('cat.mat','block_category');

[embed_data,bits_frame1,Seq_r1,sae1] = intra_4(Seq,block_category);
disp(embed_data);
...
%ct = category_table
%%
function [h_data,bits_frame,Seq_r,total_sae] = intra_4(Seq,block_category)
        ...
        for i = m:4:m+15
            ...
            for j = n:4:n+15
                ...
                elseif (i==1)       % Horz prediction
                    ...
                    [inf,icp_r,bits] = code_block(icp,block_category(row,col));
...
function [info,err_r,bits_mb] = code_block(err,category)

global QP 
bits_mb = ''; 
[n,m] = size(err);
err_r=zeros(n,m);
%Predict mode for each Macroblock 4x4 
for i = 1:4:n
    for j = 1:4:m
        c(i:i+3,j:j+3) = integer_transform(err(i:i+3,j:j+3));
        cq(i:i+3,j:j+3) = quantization(c(i:i+3,j:j+3),QP);
        %==================================================================
        [info,cq(i:i+3,j:j+3)]=embed_data_lin(cq(i:i+3,j:j+3),category);
